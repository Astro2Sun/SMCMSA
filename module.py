...
def ff2(inputs, num_units, scope="positionwise_feedforward2"):  # 全连接层*2 -> 残差链接（全连接结果加上一开始的输入） -> Normalize
    '''position-wise feed forward net. See 3.3
    ...
    with tf.variable_scope(scope):
        # Inner layer
        outputs = tf.layers.dense(inputs, num_units[0])
        outputs = tf.squeeze(outputs, 2)

        # Outer layer
        outputs = tf.layers.dense(outputs, num_units[1], activation=tf.nn.tanh)
