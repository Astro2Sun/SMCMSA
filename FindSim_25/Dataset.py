...
#         return traindata, testdata
    ...
    def setdata_for_Tdowndim(self, result,testdata_target,total_pred,traindata_target=0):
            ttestdata = {'ID': [], 'T': [], 'L': [], 'F': [], 'PreL': []}
            for i in range(len(result)):
                ttestdata['ID'].append(testdata_target['ID'][i])
                ttestdata['T'].append(result[i])
                ttestdata['L'].append(testdata_target['L'][i])
                ttestdata['F'].append(testdata_target['F'][i])
                ttestdata['PreL'].append(total_pred[i])
            return ttestdata

    def setdata_for_Adowndim(self, result,testdata_target,total_pred,traindata_target=0):
            atestdata = {'ID': [],  'A': [], 'L': [], 'F': [],'PreL': []}
            for i in range(len(result)):
                atestdata['ID'].append(testdata_target['ID'][i])
                atestdata['A'].append(result[i])
                atestdata['L'].append(testdata_target['L'][i])
                atestdata['F'].append(testdata_target['F'][i])
                atestdata['PreL'].append(total_pred[i])
            return atestdata

    def setdata_for_Vdowndim(self, result,testdata_target,total_pred,traindata_target=0):
            vtestdata = {'ID':[], 'V':[],  'L':[], 'F':[],'PreL': []}
            for i in range(len(result)):
                vtestdata['ID'].append(testdata_target['ID'][i])
                vtestdata['V'].append(result[i])
                vtestdata['L'].append(testdata_target['L'][i])
                vtestdata['F'].append(testdata_target['F'][i])
                vtestdata['PreL'].append(total_pred[i])
            return vtestdata
